...
## 2026-07-01｜移动端混音试听双轨播放

- **现象**：PWA/移动端点击「开始试听」后 UI 卡住，仅播客有声、BGM 无声
- **根因**：`await podcast.play()` 后再 `await bgm.play()` 在 iOS 会丢失用户手势；延迟 autoplay 亦无法同时解锁两路 audio
- **修复**：两路 `play()` 同 tick 并行；移动端等双轨 `canplay` 后由用户点播放钮触发；补 BGM error 与 `playsInline`
